...
class Game:
    def __init__(self):
        ...
        self.bang_sound.set_volume(0.1)  # 默认为0.1
        self.start_sound = pygame.mixer.Sound(r"music\start.wav")
        # 队友加载和吃道具音效
        self.add_sound = pygame.mixer.Sound(r"music\add.wav")
        # 子弹打出音效（正式）
        self.attack_sound = pygame.mixer.Sound(r"music\attack.wav")
        # 获取道具音效（正式）
        self.get_props_sound = pygame.mixer.Sound(r"music\get_props.wav")
        # 道具出现音效（正式）
        self.prop_sound = pygame.mixer.Sound(r"music\prop.wav")
        # 炸弹爆炸特效（正式）
        self.prop_boom_sound = pygame.mixer.Sound(r"music\prop_boom.wav")
        # 打到墙上的音效（正式）
        self.wall_sound = pygame.mixer.Sound(r"music\wall.wav")
    ...
    # 设置函数:目前只实现修改音量
    def setUp(self, rank):
        self.bang_sound.set_volume(rank)
    ...
    def game_running_singled_out(self, checkpoint):
        # 创建地图
        map_num = [[0] * 26 for i in range(26)]
        map_num[23][11] = map_num[23][12] = map_num[23][13] = map_num[23][14] = map_num[24][11] = \
            map_num[24][14] = map_num[25][11] = map_num[25][14] = 1
