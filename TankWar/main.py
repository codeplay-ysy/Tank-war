...
def main():
    ...
    # 创建加载界面的图片
    init_image_all = [None] * 127
    for i in range(1, 64):
        init_image_all[i] = pygame.image.load(r"init/gif" + str(i) + ".png")
    for j in range(64, 127):
        init_image_all[j] = pygame.image.load(r"init/gif" + str(j - 63) + ".png")
    # 显示加载界面（其中一张图片）
    surface.blit(init_image_all[1], (150, 80))
    pygame.display.flip()
    now_i = 1
    ...
    for i in range(1, 36):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                sys.exit()
        # 显示加载界面
        surface.blit(init_image_all[now_i], (150, 80))  # 显示图片
        now_i += 1  # 将图片的索引换位下一张
        ...
        level_mode_menu.add.button('', Level_mode, i, background_color=background_image,
                                   ...
                                   border_width=(10))
        # 显示加载界面
        surface.blit(init_image_all[now_i], (150, 80))
        now_i += 1
        pygame.display.flip()
        # 无尽模式菜单添加标签、间隔、按钮
    ...
    # Set Up菜单创建
    set_up_menu = pygame_menu.Menu('Select a suitable volume', 750, 630, theme=checkpoint_theme)
    set_up_menu.add.button('Minimum volume', set_up, 0.2)
    set_up_menu.add.button('Moderate volume', set_up, 0.5)
    set_up_menu.add.button('Maximum volume', set_up, 1.0)
    set_up_menu.add.button('Mute', set_up, 0.0)
    ...
    main_menu.add.button('Set up', set_up_menu)
